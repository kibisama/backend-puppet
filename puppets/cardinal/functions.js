    ...
    async getProductDetails(page) {
      try {
        ...
        const els = await page.waitForElements([
          ...
          _xPaths.form,
          _xPaths.strength,
          _xPaths.orangeBookCode,
          _xPaths.lastOrdered,
          _xPaths.estNetCost,
          _xPaths.netUoiCost,
          _xPaths.rebateEligibleIcon,
          _xPaths.returnableIcon,
          _xPaths.stock,
          _xPaths.stockStatus,
        ]);
        if (els) {
          const result = {};
          result.name = (await page.getInnerTexts(_xPaths.name))[0];
          result.genericName = (
            await page.getInnerTexts(_xPaths.genericName)
          )[0];
          result.img = String(
            await (await els[2].getProperty("src")).jsonValue()
          ).trim();
          result.ndc = (await page.getInnerTexts(_xPaths.ndc))[0];
          result.cin = (await page.getInnerTexts(_xPaths.cin))[0];
          result.upc = (await page.getInnerTexts(_xPaths.upc))[0];
          result.gtin = (await page.getInnerTexts(_xPaths.gtin))[0];
          result.brandName = (await page.getInnerTexts(_xPaths.brandName))[0];
          result.mfr = (await page.getInnerTexts(_xPaths.mfr))[0];
          result.amu = (await page.getInnerTexts(_xPaths.amu))[0];
          result.size = (await page.getInnerTexts(_xPaths.size))[0];
          result.form = (await page.getInnerTexts(_xPaths.form))[0];
          result.strength = (await page.getInnerTexts(_xPaths.strength))[0];
          result.orangeBookCode = (
            await page.getInnerTexts(_xPaths.orangeBookCode)
          )[0];
          result.lastOrdered = (
            await page.getInnerTexts(_xPaths.lastOrdered)
          )[0];
          result.estNetCost = (await page.getInnerTexts(_xPaths.estNetCost))[0];
          result.netUoiCost = (await page.getInnerTexts(_xPaths.netUoiCost))[0];
          result.rebateEligibleIcon = (
            await page.getInnerTexts(_xPaths.rebateEligibleIcon)
          )[0];
          result.returnableIcon = (
            await page.getInnerTexts(_xPaths.returnableIcon)
          )[0];
          result.stock = (await page.getInnerTexts(_xPaths.stock))[0];
          result.stockStatus = (
            await page.getInnerTexts(_xPaths.stockStatus)
          )[0];
          result.rx = (await page.getInnerTexts(_xPaths.rx))[0];
          result.deaSchedule = (
          ...
          result.medispanGpi = (
            await page.getInnerTexts(_xPaths.medispanGpi)
          )[0];
          result.gcnSequence = (
            await page.getInnerTexts(_xPaths.gcnSequence)
          )[0];
          result.gcn = (await page.getInnerTexts(_xPaths.gcn))[0];
          result.unitOfMeasure = (
            await page.getInnerTexts(_xPaths.unitOfMeasure)
          )[0];
          result.refrigerated = (
            await page.getInnerTexts(_xPaths.refrigerated)
          )[0];
          result.serialized = (await page.getInnerTexts(_xPaths.serialized))[0];
          console.log(result);
        }
      } catch (e) {
        console.log(e);
